...
▣ 물리적 보안 : 건물 및 사무실 출입 통제 지침, 전산실 관리 지침, 정보 시스템 보호 설치 및 관리 지침
▣ 기술적 보안 : 사용자 인증, 접근 제어, PC, 서버, 네트워크, 응용 프로그램, 데이터(DB) 등의 보안 지침 정의
```
</div>

### 48. 리눅스 커널 및 로그인 관련 로그 파일 정리
<div style={{marginLeft:'2.5rem'}}>
```bash
▣ /dev/console     : 커널 관련 메시지를 저장하지 않고 관리자에게 직접 출력하는 장치
▣ /var/log/wtmp    : 성공한 로그인 및 로그아웃, 시스템 시작/종료 기록
▣ /var/run/utmp    : 현재 로그인한 사용자 상태 기록
▣ /var/log/btmp    : 실패한 로그인 시도 기록
▣ /var/log/lastlog : 마지막으로 성공한 로그인 시간 및 원격지 정보 기록
```
</div>

### 49. 침입 탐지 시스템(IDS: Intrusion Detection System)
<div style={{marginLeft:'2.5rem'}}>
```bash
▣ 컴퓨터 시스템의 비정상적인 사용, 오용, 남용 등을 실시간으로 탐지하는 시스템
```
</div>
<div style={{marginLeft:'3.5rem'}}> IDS의 필요성 </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 방화벽만으로 내부 사용자 불법 행위 및 외부 해킹을 100% 차단할 수 없음
▣ 해커의 침입 패턴을 추적하고 유해 정보를 감시하여 실시간 대응 필요
```
</div>
<div style={{marginLeft:'3.5rem'}}> 탐지 기법 - 오용 탐지(Misuse Detection)  </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 사전에 정의된 공격 패턴을 기반으로 탐지
▣ 알려진 공격에는 효과적이나 새로운 공격에는 취약
```
</div>
<div style={{marginLeft:'3.5rem'}}> 탐지 기법 - 이상 탐지(Anomaly Detection)  </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 평균적인 시스템 상태를 기준으로 비정상적인 행위 탐지
▣ 새로운 공격도 감지 가능하지만 오탐(FP)이 발생할 수 있음
```
</div>
<div style={{marginLeft:'3.5rem'}}>  IDS의 종류 - 호스트 기반 침입 탐지 시스템 (HIDS: Host-Based IDS) </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 개별 시스템 내부를 감시 및 분석
▣ 접근 기록 및 시스템 변경 사항 추적
▣ 대표 도구: OSSEC, md5deep, AIDE, Samhain 등
```
</div>
<div style={{marginLeft:'3.5rem'}}>  IDS의 종류 - 네트워크 기반 침입 탐지 시스템 (NIDS: Network-Based IDS) </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 네트워크 트래픽을 감시하여 외부 침입 탐지
▣ 대표 도구 : Snort, Zeek 등
▣ 탐지 대상 : 서비스 거부 공격(DoS), 포트 스캔, 기타 악의적 네트워크 활동
```
</div>

<div style={{marginLeft:'3.5rem'}}> IDS의 설치 위치 및 역할 </div>
<div style={{marginLeft:'3.5rem'}}>
```bash
▣ 라우터 앞     : 네트워크로 유입되는 모든 공격 탐지
▣ 라우터 뒤     : 라우터 필터링을 통과한 공격 탐지
▣ 방화벽 뒤     : 내부에서 외부로 향하는 공격 탐지
▣ 내부 네트워크 : 내부 시스템 간의 해킹 공격 탐지
▣ DMZ          : 외부 공격 및 내부 공격으로부터 서버 보호
```
</div>
